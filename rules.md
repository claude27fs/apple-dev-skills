...
1. **数量**：每天挖掘 5 款流行的 skill
2. **去重**：同一 skill 不重复收录
3. **相似合并**：相似的 skill 只保留 Stars 最多的
4. **必须是 AI Agent Skill**：必须是供 AI Agent（Claude Code、Cursor、Codex、OpenClaw 等）调用的工具/Skills/MCP Server
5. **与苹果开发相关**：skill 必须与 Apple 平台开发相关（iOS/macOS/watchOS/tvOS/Swift/SwiftUI/AppKit/UIKit 等）
